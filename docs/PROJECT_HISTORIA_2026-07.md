# Nuta — ustalenia projektowe (ARCHIWUM, lipiec 2026)

> **To zapis historyczny, NIE aktualny kontekst projektu.** Ostatnia zmiana treści:
> 18.07.2026. Aktualne zasady i opis projektu: [`AGENTS.md`](../AGENTS.md); decyzje
> nadal obowiązujące: [`PROJECT.md`](../PROJECT.md).
>
> Co się od lipca rozjechało — nie kieruj się tym z tego pliku:
> - **Brak ListenBrainz.** Dokument zna tylko Spotify, a dziś to dwa niezależne źródła
>   danych (`DataSource`: SPOTIFY/LISTENBRAINZ) plus MusicBrainz jako katalog
>   wyszukiwania i scrobbling do ListenBrainz.
> - **Brak SoundCloud** jako alternatywnego źródła audio (`AudioSource`).
> - **„Strategia Linux-first" jest odwrócona** — priorytetem jest Android, desktop
>   odłożony (patrz `docs/ANDROID_PLAN.md`).
> - **JCEF porzucony** na rzecz WebView2 (`docs/WEBVIEW2_MIGRATION_PLAN.md`), więc
>   opisy `SpotifyCookieSessionStore`, wolumenu `nuta-session` i `token.test.json`
>   dotyczą rozwiązania, którego już nie ma.
> - **Skrypty `.ps1` deploya zastąpione przez `.py`** (`scripts/deploy-android.py`).
> - Profile klienta YouTube z tego okresu (WEB/TVHTML5/ANDROID/IOS) już nie istnieją —
>   dziś `enum { AUTO, VISIONOS, ANDROID_VR }`, patrz `docs/sabr-blocker/`.
>
> Wartość tego pliku: pokazuje, **jak myśleliśmy w lipcu** i dlaczego pewne decyzje
> zapadły (wykluczenie OAuth, rezygnacja z iOS, zasady wokół `sp_dc`).

Ten dokument był źródłem kontekstu dla kolejnych modeli AI i deweloperów pracujących nad projektem.

Szczegółowy plan pierwszej fazy znajduje się w [docs/PHASE_1_LINUX_GUI.md](PHASE_1_LINUX_GUI.md).
...
Szczegółowy plan integracji sesji web-playera znajduje się w [`docs/PHASE_2_SPOTIFY_WEB_SESSION.md`](PHASE_2_SPOTIFY_WEB_SESSION.md).

Szczegółowy plan wyszukiwania YouTube i pozyskania strumienia audio znajduje się w [`docs/PHASE_3_YOUTUBE_STREAM.md`](PHASE_3_YOUTUBE_STREAM.md).
